...
def analyze_image(image):
    ...
    points = detect_edge_features(image, EDGE_REGIONS[0:4])
    ...
    if debug:
        with open(os.path.join(outdir, '{}-{}.tsv'.format(id, page)), 'w') as f:
            for i, v, scr, b in res:
                f.write('{}\t{}\t{}\t{}\n'.format(i, v, scr, b))
        with open(os.path.join(outdir, '{}-{}-log.txt'.format(id, page)), 'w') as f:
            f.write('\n'.join(log))
    else:
        with open(os.path.join(outdir, '{}-{}.tsv'.format(id, page)), 'w') as f:
            for i, v, _, __ in res:
...
def main():
    ...
    if not os.path.isdir(args.templatedir):
        ...
        templ = load_image(f)
        logging.info('Loaded templatefile "{}" for registration.'.format(f))
        ...
        marks = []
        for form in TEMPLATES_FORMS:
            f = os.path.join(args.templatedir, form)
            m = load_svg_rects(f, templ.image.shape)
            logging.info('found {} marks in template {}'.format(len(m), f))
            marks.append((m))
